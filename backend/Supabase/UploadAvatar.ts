...
export const pickAndUploadAvatar = async (walletAddress: string): Promise<string | null> => {
  // 1. Request media library permissions
  const permission = await ImagePicker.requestMediaLibraryPermissionsAsync();
  if (!permission.granted) {
    Alert.alert("Permission Denied", "You need to allow access to your photos to update your avatar.");
    return null;
  }
